...
void TransmitRequestPacket::setData( unsigned char applicationID, std::vector<unsigned char> zigbeeAddress64Bit, std::vector<unsigned char> data, unsigned char frameID)
{
    ...
    setRFData(zigbeeAddress64Bit, applicationData, frameID);
}

void TransmitRequestPacket::setRFData(std::vector<unsigned char> zigbeeAddress64Bit, std::vector<unsigned char> applicationData, unsigned char frameID)
{
    std::vector<unsigned char> rfData;
    rfData.push_back(0x10);     // TransmitRequest has Frame Type 10
    rfData.push_back(frameID);     // Frame ID, if set to 0 no ack will be sent. Else an ack with this ID will be sent back.
...
unsigned char TransmitRequestPacket::getFrameID() const throw (ZbCorruptedFrameData)
{
